...
void main( void ) {
	...
	vec2 scale_factor;
	scale_factor.x = 400.5 * color_1.z;
	...
	neigh_pos = mix(neigh_pos, vec2(color_1.z,color_1.x), sin(u_timeS*M_2PI * .01) * 0.002*  (2.0 * color_1.z - 1.0));

	color_2 = texture2D(u_state, neigh_pos);

	float mix_amount = sin(u_timeS*M_2PI * (2.7 + (1.15 * (color_2.z - 0.5))))*0.002;
	my_next_color = mix(color_2,color_1,mix_amount);

	float neighbors_weight = (1.3 * (sin(u_timeS * M_2PI * 0.01 + M_PI) + 1.2));
